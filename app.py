...
st.set_page_title_config = st.set_page_config(page_title="Automação Caderno de Campo - PEPAC", layout="centered")
...
def process_caderno(pu_file, template_file):
    ...
    cult_map = {
        ...
        989: '989 - Elp Vala De Rega Ou Drenagem - Área Útil',
        ...
        939: '939 - Ep- Bosquete E Formações ReliquiaisÁrea Útil'
    }
    ...
    df_final['Cultura/Variedade ou casta'] = df_culturas['culCodigo'].map(cult_map).fillna(df_culturas['culCodigo'].astype(str) + " - [Outra Cultura]")
    ...
    df_maa['abbrev'] = df_maa['intCodigo'].map(code_mapping).fillna(df_maa['intCodigo'])
    ...
    df_final = df_final.drop(columns=['Intervenções'])
    ...
    def get_rega(cultura):
        ...
        if "amendoal" in cult_str or "olival" in cult_str or "vinha" in cult_str or "noz" in cult_str or "tomate" in cult_str or "melão" in cult_str:
            return "gota-a-gota"
        return "Não"
